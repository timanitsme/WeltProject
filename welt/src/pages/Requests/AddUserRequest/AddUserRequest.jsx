...
import {
    ...
    useGetMyProjectsPaginatedQuery, useGetUsersExceptMeQuery
...
import styles from "../../Admin/AdminRequests/AddRequest/AddRequest.module.scss";
...
export default function AddUserRequest(){
    ...
    const [createRequest, { isLoading, isSuccess, isError, error }] = useCreateRequestMutation();
    ...
    const { data: projectsData, isLoading: projectsLoading, error: projectsError } = useGetMyProjectsPaginatedQuery({
    ...
    const paths =[
        ...
        {title: "Мои заявки", path: "/requests/my"},
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await createRequest({ subject: subject, description: description, project_id: projectId, receiver_id: receiverId }).unwrap();
            setSubject('');
            setDescription('');
            setReceiverId('')
            setProjectId('')
